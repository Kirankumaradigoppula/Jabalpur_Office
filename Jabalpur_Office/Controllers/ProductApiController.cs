    ...
    public class ProductApiController : BaseApiController
    {
        ...
        public IActionResult GetUserLocationMasterDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactUserLocationMasterDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
            ...
            }, nameof(GetUserLocationMasterDetails), out _, skipTokenCheck: false));
        ...
        public IActionResult CrudUserLocationMasterDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudUserLocationMasterDetails", paramList.ToArray());
                SetOutputParams(pStatus, pMsg,  outObj);
                return outObj;

            }, nameof(CrudUserLocationMasterDetails), out _, skipTokenCheck: false));
        ...
        public IActionResult GetAlbumCategoryDetails([FromBody] object input)
        {
            ...
            }, nameof(GetAlbumCategoryDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudAlbumCategoryDetails([FromForm] string input, [FromForm] List<IFormFile> files)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                if (flag =="SAVE")
                {
                    ...
                    {
                        outObj.StatusCode = 500;
                        outObj.Message = "Album Cover file is required.";
                        outObj.LoginStatus = pJWT_LOGIN_NAME;
                        return outObj;
                    }
                    if (files.Count > 1)
                    ...
                    {
                        outObj.StatusCode = 400;
                        outObj.Message = $"Invalid file type ({ext}). Allowed: JPG, PNG";
                        outObj.LoginStatus = pJWT_LOGIN_NAME;
                        return outObj;
                    }
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudAlbumCategoryDetails", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
                if (outObj.StatusCode == 200)
                {
                    if (flag == "SAVE" || flag == "UPDATE" && (files != null && files.Count > 0) )
                    {
                        ...
                        string baseFolder = Path.Combine("image", $"MP_{pJWT_MP_SEAT_ID}", "Albums");
                        ...
                        string FileName = $"AlbumCover_{outObj.RetID}{ext}";
            ...
            }, nameof(CrudAlbumCategoryDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudAlbumDetails([FromForm] string input, [FromForm] List<IFormFile> files)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                if (flag == "SAVE" || flag == "UPDATE")
                {
                    ...
                    {
                        outObj.StatusCode = 500;
                        outObj.Message = "Album file is required.";
                        outObj.LoginStatus = pJWT_LOGIN_NAME;
                        return outObj;
                    }
                    if (files != null && files.Count > 0)
                    {
                        ...
                        string[] allowedExt = new[] { ".jpg", ".jpeg", ".JPEG", ".png" };
                        foreach (var file in files)
                        {
                            string FileName = string.Empty;
                            string FilePath = string.Empty;
                            ...
                            if (file.Length > 0)
                            {
                                ...
                                {
                                    outObj.StatusCode = 500;
                                    outObj.Message = "Only JPG, PNG files are allowed.";
                                    outObj.LoginStatus = pJWT_LOGIN_NAME;
                                    return outObj;
                                }
                            ...
                            DataTable dt = _core.ExecProcDt("ReactCrudAlbumDetails", paramList.ToArray());
                            SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
                            if (outObj.StatusCode == 200)
                            {
                                try
                                {
                                    var storageRoot = _settings.BasePath;
                                    string baseFolder = string.Empty;
                                    baseFolder = $"image/MP_{pJWT_MP_SEAT_ID}/Albums";
                                    string finalFolder = Path.Combine(storageRoot, baseFolder);
                                    if (!Directory.Exists(finalFolder))
                                    {
                                        Directory.CreateDirectory(finalFolder);
                                    ...
                                    string relativePath = $"image/MP_{pJWT_MP_SEAT_ID}/Albums/{FileName}";

                                    string vQryUpdateStatus = $"UPDATE ALBUM_DETAILS SET IMAGE=N'" + FileName + "' ,IMAGE_PATH=N'" + relativePath + "' WHERE MP_SEAT_ID='" + pJWT_MP_SEAT_ID + "' AND ALBUM_DET_ID='" + outObj.RetID + "' ";
                                    _core.ExecNonQuery(vQryUpdateStatus);
                                }
                                catch(Exception Ex)
                                {
                                    string delQry = $"DELETE FROM ALBUM_DETAILS WHERE MP_SEAT_ID={pJWT_MP_SEAT_ID} AND ALBUM_DET_ID='" + outObj.RetID + "' ";
                                    _core.ExecNonQuery(delQry);
                                    outObj.StatusCode = 500;
                                    outObj.Message = "File upload failed. Record has been removed.";
                                    outObj.LoginStatus = pJWT_LOGIN_NAME;
                    ...
                    if (flag == "DELETE" || flag == "DELETE_IMAGE")
                    {
                        ...
                        pImagePath = Convert.ToString(
                                  _core.ExecScalarText(
                                    ...
                                    new SqlParameter("@MP_SEAT_ID", pJWT_MP_SEAT_ID) ,
                                    new SqlParameter("@ALBUM_CAT_ID",data["ALBUM_CAT_ID"]?.ToString()),
                                    new SqlParameter("@ALBUM_DET_ID",data["ALBUM_DET_ID"]?.ToString())
            ...
            }, nameof(CrudAlbumDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudAnniversaryBirthdayDetails([FromForm] string input, [FromForm] List<IFormFile> files)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                if (files != null && files.Count > 0 && (flag == "DOB_UPDATE" || flag == "DOA_UPDATE"))
                {
                    ...
                    // Prepare storage paths
                    string storageRoot = _settings.BasePath;
                    string baseFolder = Path.Combine("image", $"MP_{pJWT_MP_SEAT_ID}", "BdayProfiles");
                    string voterNoSafe = data["VOTERNO"]?.ToString().Replace("/", "_") ?? "Unknown";

                    // Set profile image name based on flag
                    pPROFILE_IMAGE = flag == "DOB_UPDATE"
                        ? $"BirthdayProfile_{voterNoSafe}{ext}"
                        : $"AnniversaryProfile_{voterNoSafe}{ext}";
        ...
        public IActionResult CrudLetterRecipientMaster([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudLetterRecipientMaster", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
                return outObj;

            }, nameof(CrudLetterRecipientMaster), out _, skipTokenCheck: false));
        ...
        public IActionResult GetLetterRecipientMasterDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactLetterRecipientMasterDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
            ...
            }, nameof(GetLetterRecipientMasterDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudConstituencyDesignationMasterDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudConstituencyDesignationMasterDetails", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
                return outObj;

            }, nameof(CrudConstituencyDesignationMasterDetails), out _, skipTokenCheck: false));
        ...
        public IActionResult GetConstituencyDesignationDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactConstituencyDesignationDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
        ...
        public IActionResult CrudAssignBoothDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudAssignBoothDetails", paramList.ToArray());
                SetOutput(pStatus, pMsg, outObj);
                return outObj;

            }, nameof(CrudAssignBoothDetails), out _, skipTokenCheck: false));
        ...
        public IActionResult CrudConstituencyMasDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudConstituencyMasDetails", paramList.ToArray());
                SetOutput(pStatus, pMsg, outObj);
                return outObj;

            }, nameof(CrudConstituencyMasDetails), out _, skipTokenCheck: false));
        ...
        public IActionResult GetWardMasterDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactWardMasterDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetWardMasterDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult GetShaktikendraMasterDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactShaktikendraMasterDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetShaktikendraMasterDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult GetBoothMasterDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactBoothMasterDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetBoothMasterDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudContactBook([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudContactBook", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
                return outObj;

            }, nameof(CrudContactBook), out _, skipTokenCheck: false));
        ...
        public IActionResult GetContactBookDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactContactBookDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetContactBookDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult GetLetterTemplateDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactLetterTemplateDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetLetterTemplateDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult GetVisitorOutwardDocumentDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                var OUTWARD_IS_DISABLE = new SqlParameter("@pOUTWARD_IS_DISABLE", SqlDbType.Bit)
                {
                    Direction = ParameterDirection.Output
                };
                paramList.Add(OUTWARD_IS_DISABLE);

                DataTable dt = _core.ExecProcDt("ReactVisitorOutwardDocumentDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);


                // Read @pTotalAmount output value safely
                if (OUTWARD_IS_DISABLE.Value != DBNull.Value)
                {
                    outObj.ExtraData["OUTWARD_IS_DISABLE"] = Convert.ToBoolean(OUTWARD_IS_DISABLE.Value);
                }
                ...
                return outObj;
            }, nameof(GetVisitorOutwardDocumentDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudVisitorOutwardDocumentDetails([FromForm] string input, [FromForm] List<IFormFile> files)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                    ...
                    pImagePath = Convert.ToString(
                         _core.ExecScalarText(
                              pQry,
                               new[] {
                            new SqlParameter("@MP_SEAT_ID", pJWT_MP_SEAT_ID),
                            new SqlParameter("@VIS_OUT_MAS_ID", data["VIS_OUT_MAS_ID"]?.ToString())
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudVisitorOutwardDocumentDetails", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
                if (outObj.StatusCode == 200)
                {
                    if (flag == "SAVE" || flag == "UPDATE" && (files != null && files.Count > 0) && files.Any() == true)
                    {
                        ...
                        // Prepare storage paths
                        string storageRoot = _settings.BasePath;
                        string baseFolder = Path.Combine("image", $"MP_{pJWT_MP_SEAT_ID}", "VIS_OUTWARD_DOCUMENT");
                        ...
                        string FileName = $"OutWard_{outObj.RetID}{ext}";
                        string relativePath = Path.Combine(baseFolder, FileName).Replace("\\", "/");
                        string fullPath = Path.Combine(finalFolder, FileName);
                        // ✅ If file already exists, delete it before saving
                        if (System.IO.File.Exists(fullPath))
        ...
        public IActionResult GetVisitorWorkStatusDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                var WORK_IS_DISABLE = new SqlParameter("@pWORK_IS_DISABLE", SqlDbType.Bit)
                {
                    Direction = ParameterDirection.Output
                };
                paramList.Add(WORK_IS_DISABLE);

                DataTable dt = _core.ExecProcDt("ReactVisitorWorkStatusDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);


                // Read @pTotalAmount output value safely
                if (WORK_IS_DISABLE.Value != DBNull.Value)
                {
                    outObj.ExtraData["WORK_IS_DISABLE"] = Convert.ToBoolean(WORK_IS_DISABLE.Value);
                }
                ...
                return outObj;
            }, nameof(GetVisitorWorkStatusDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudVisitorWorkStatusDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudVisitorWorkStatusDetails", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
        ...
        public IActionResult GetDistrictMasDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactDistrictMasDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetDistrictMasDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudDistrictMasDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudDistrictMasDetails", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
        ...
        public IActionResult CrudVoterContactDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudVoterContactDetails", paramList.ToArray());
                SetOutput(pStatus, pMsg, outObj);
                return outObj;

            }, nameof(CrudVoterContactDetails), out _, skipTokenCheck: false));
        ...
        public IActionResult CrudVisitorMasterDetails([FromForm] string input, [FromForm] List<IFormFile> files)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                if (outObj.StatusCode == 200)
                {
                    if (files?.Any() == true && (flag == "SAVE" || flag == "UPDATE" ))
                    {
                        if (files.Count > 1)
                        {
                            outObj.StatusCode = 500;
        ...
        public IActionResult CrudVisitorDetails([FromForm] string input, [FromForm] List<IFormFile> VIS_INWARD_FILE, [FromForm] string VIS_WEBCAM_FILE=null)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                // ---------------------------------------------
                var (outObj, rawData) = PrepareWrapperAndData<WrapperCrudObjectData>(
                    string.IsNullOrEmpty(input) ? new { } : ApiHelper.ToObject(input)
                );

                var data = ApiHelper.ToObjectDictionary(rawData);
                var filterKeys = ApiHelper.GetFilteredKeys(data);

                string flag = data.GetValueOrDefault("FLAG")?.ToString() ?? "";
                string pVIS_REASON = data.GetValueOrDefault("VIS_REASON")?.ToString() ?? "";

                string pInwardFilePath = string.Empty;
                string pWebCamFilePath = string.Empty;


                // ---------------------------------------------
                // STEP 2 — BEFORE DELETE: fetch old file paths
                // ---------------------------------------------

                if (flag =="DELETE" || flag == "DELETE_IMAGE")
                {
                    string pInwardQry = @"SELECT VIS_INWARD_FILE_PATH FROM VISITOR  WHERE MP_SEAT_ID = @MP_SEAT_ID AND VIS_SRNO=@VIS_SRNO";
                    // Get old MEDIA_DATE from DB (before update)
                     pInwardFilePath = Convert.ToString(
                         _core.ExecScalarText(
                              pInwardQry,
                               new[] {
                                new SqlParameter("@MP_SEAT_ID", pJWT_MP_SEAT_ID),
                                new SqlParameter("@VIS_SRNO", data["VIS_SRNO"]?.ToString())

                               }
                          )
                       );
                    //string pWebCamQry = @"SELECT VIS_WEBCAM_FILE_PATH FROM VISITOR  WHERE MP_SEAT_ID = @MP_SEAT_ID AND VIS_SRNO=@VIS_SRNO";
                    //// Get old MEDIA_DATE from DB (before update)
                    //pWebCamFilePath = Convert.ToString(
                    //    _core.ExecScalarText(
                    //         pWebCamQry,
                    //          new[] {
                    //            new SqlParameter("@MP_SEAT_ID", pJWT_MP_SEAT_ID),
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudVisitorDetails", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
                ...
                // ---------------------------------------------
                // STEP 4 — SAVE & UPDATE LOGIC
                // ---------------------------------------------
                if (flag == "SAVE" || flag == "UPDATE")
                {
                    // -----------------------------------------
                    // 4A — SAVE ONLY: GET REASON_MAX & INWARD NO
                    // -----------------------------------------
                    if (flag == "SAVE")
                    {
                        string pQry =
                            @"SELECT ISNULL(MAX(REASON_MAX_SRNO),0)+1 AS REASON_MAX,
                               REASON_SHORT_NAME
                      FROM REASON_MASTER
                      WHERE REASON_MAS_ID=@VIS_REASON
                      GROUP BY REASON_SHORT_NAME";

                        DataTable dt2 = _core.ExecDtText(
                            pQry,
                            new[] { new SqlParameter("@VIS_REASON", pVIS_REASON) }
                        );

                        if (dt2.Rows.Count > 0)
                        {
                            string reasonMax =
                                $"{dt2.Rows[0]["REASON_SHORT_NAME"]}/{dt2.Rows[0]["REASON_MAX"]}";
                            outObj.ExtraData["REASON_MAXSRNO"] = reasonMax;
                        }

                        string inwardQry =
                        @"SELECT VIS_INWARD_LETTER_NO FROM VISITOR
                          WHERE MP_SEAT_ID=@MP_SEAT_ID AND VIS_SRNO=@VIS_SRNO";

                        string inwardNo = Convert.ToString(
                            _core.ExecScalarText(
                                inwardQry,
                                new[] {
                            new SqlParameter("@MP_SEAT_ID", pJWT_MP_SEAT_ID),
                            new SqlParameter("@VIS_SRNO", outObj.RetID)
                                })
                        );

                        outObj.ExtraData["VIS_INWARD_LETTER_NO"] = inwardNo;
                    }
                    // -----------------------------------------
                    // 4B — INWARD FILE UPLOAD
                    // -----------------------------------------
                    if (VIS_INWARD_FILE != null && VIS_INWARD_FILE.Any())
                    {
                        if (VIS_INWARD_FILE.Count > 1)
                        {
                            outObj.StatusCode = 500;
                            outObj.Message = "You can upload a maximum of ONE file only.";
                            return outObj;
                        }

                        var file = VIS_INWARD_FILE.First();
                        string ext = Path.GetExtension(file.FileName)?.ToLower() ?? "";

                        string[] allowedExt = { ".jpg", ".jpeg", ".png", ".pdf", ".doc", ".docx", ".xls", ".xlsx" };

                        if (!allowedExt.Contains(ext))
                        {
                            outObj.StatusCode = 500;
                            outObj.Message = "Only jpg, jpeg, png, pdf, doc, docx, xls, xlsx files are allowed.";
                            return outObj;
                        }
                        ...
                        string fileName = $"{pJWT_MP_SEAT_ID}_{outObj.RetID}{ext}";
                        string relativePath = Path.Combine(baseFolder, fileName).Replace("\\", "/");
                        string fullPath = Path.Combine(finalFolder, fileName);
                        ...
                        string vQryUpdateStatus = $"UPDATE VISITOR SET VIS_INWARD_DOCUMENT='" + fileName + "' ,VIS_INWARD_FILE_PATH='" + relativePath + "' WHERE MP_SEAT_ID='" + pJWT_MP_SEAT_ID + "' AND VIS_SRNO='" + outObj.RetID + "' ";
                        _core.ExecNonQuery(vQryUpdateStatus);
                    }

                    // -----------------------------------------
                    // 4C — WEBCAM IMAGE SAVE
                    // -----------------------------------------
                    if (!string.IsNullOrWhiteSpace(VIS_WEBCAM_FILE))
                    {
                        var parts = VIS_WEBCAM_FILE.Split(',');
                        if (parts.Length == 2)
                        {
                            byte[] imgBytes = Convert.FromBase64String(parts[1]);

                            string storageRoot = _settings.BasePath;
                            string baseFolder = Path.Combine("image", $"MP_{pJWT_MP_SEAT_ID}", "WebCamImg");
                            string finalFolder = Path.Combine(storageRoot, baseFolder);
                            Directory.CreateDirectory(finalFolder);

                            string fileName = $"{pJWT_MP_SEAT_ID}_WebCam_{outObj.RetID}.jpg";
                            string relativePath = Path.Combine(baseFolder, fileName).Replace("\\", "/");
                            string fullPath = Path.Combine(finalFolder, fileName);
                            ...
                            System.IO.File.WriteAllBytes(fullPath, imgBytes);

                            string vQryUpdateStatus = $"UPDATE VISITOR SET VIS_WEBCAM_IMG='" + fileName + "' ,VIS_WEBCAM_FILE_PATH='" + relativePath + "' WHERE MP_SEAT_ID='" + pJWT_MP_SEAT_ID + "' AND VIS_SRNO='" + outObj.RetID + "' ";
                            _core.ExecNonQuery(vQryUpdateStatus);
                        }
                    }
                }
                ...
                if (flag == "DELETE" || flag == "DELETE_IMAGE")
                {
                    DeletePhysicalFile(pInwardFilePath);
                    DeletePhysicalFile(pWebCamFilePath);
                }


                return outObj;

            }, nameof(CrudVisitorDetails), out _, skipTokenCheck: false));
        ...
        private void DeletePhysicalFile(string relativePath)
        {
            ...
            string physicalPath = Path.Combine(_settings.BasePath, relativePath.Replace("/", "\\"));
        ...
        public IActionResult GetAssemblyBoothWisePPPCSummaryDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactAssemblyBoothWisePPPCSummaryDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetAssemblyBoothWisePPPCSummaryDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult GetPPPCSummaryDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactPPPCSummaryDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetPPPCSummaryDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult GetSingleVoterDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactSingleVoterDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetSingleVoterDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult GetVoterIdCardDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactVoterIdCardDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
        ...
        public IActionResult GetKeyVotersDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactKeyVotersDetails", paramList.ToArray());
                ApiHelper.SetDataTableListOutput(dt, outObj);
                SetOutput(pStatus, pMsg, outObj);
                ...
                return outObj;
            }, nameof(GetKeyVotersDetails), out _, skipTokenCheck: false));
        }
        ...
        public IActionResult CrudKeyVotersDetails([FromBody] object input)
        {
            return Ok(ExecuteWithHandling(() =>
            {
                ...
                DataTable dt = _core.ExecProcDt("ReactCrudKeyVotersDetails", paramList.ToArray());
                SetOutputParamsWithRetId(pStatus, pMsg, pRetId, outObj);
                return outObj;
